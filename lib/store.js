...
export async function addMonitor(member) {
  ...
  list.push({
    ...
    note: member.note || "",
    betThreshold: Number(member.betThreshold) || 0, // 0 = 無門檻（任何下注都通知）
...
export async function updateMonitor(id, updates) {
  ...
  if (index === -1) throw new Error("找不到該會員");
...
// ===== 活動 Session 追蹤 =====
// 用於「第一次投注 → 閒置 30 分鐘」的活動結束偵測

const KEY_SESSION = "session:";

export async function getSession(memberId) {
  return await kv.get(KEY_SESSION + memberId);
}

export async function setSession(memberId, session) {
  // 7 天後自動清除（防止殘留）
  await kv.set(KEY_SESSION + memberId, session, { ex: 7 * 86400 });
}

export async function clearSession(memberId) {
  await kv.del(KEY_SESSION + memberId);
}
